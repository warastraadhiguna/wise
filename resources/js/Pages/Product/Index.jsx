...
const Index = ({ title, products, searchingTextProps, productCategories, brands, units }) => {
    ...
    const [perPage, setPerPage]  = useState(products.per_page);
    ...
    const actionForm = (e) => {
        ...
        if (dataProps.id) {
            router.put(`product/${dataProps.id}`, dataProps, {
                onSuccess: () => {
                    resetForm(true);
                },
                onError: () => {
    ...
    const handleDelete = () => {
        ...
        router.delete(`/product/${dataProps.id}`,  {
            onFinish: () => {
                ...
                <table className="w-full text-sm text-left rtl:text-right text-black dark:text-gray-400">
                    ...
                    <tbody>
                        {products.data.map((product, i) => (
                            <tr
                                key={i}
                                className={`bg-white border-b dark:bg-gray-800 dark:border-gray-700  ${product.deleted_at? "line-through bg-yellow-50" : ""}`}
                            >
                                <td
                                    scope="row"
                                ...
                                <td className="px-6 py-4">{product.product_category? product.product_category.name : "-"}</td>
                                <td className="px-6 py-4">{product.brand? product.brand.name : "-"}</td>
                                <td className="px-6 py-4">{product.unit.name}</td>                                
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center justify-center gap-2">
                                        <FaPencilAlt
                                            size={20}
                                            color={ product.deleted_at? "#c2bc42" : "green"}
                                        ...
                                        <FaRegTrashAlt
                                            size={20}
                                            color={ product.deleted_at? "#e18859" : "red"}
                                            className="cursor-pointer"
                                            onClick={() =>
