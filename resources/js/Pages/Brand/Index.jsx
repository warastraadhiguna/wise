...
const Index = ({ title, brands, searchingTextProps }) => {
    ...
    const [perPage, setPerPage]  = useState(brands.per_page);
    ...
    const actionForm = (e) => {
        ...
        if (dataProps.id) {
            router.put(`brand/${dataProps.id}`, dataProps, {
                onSuccess: () => {
                    resetForm(true);
                },
                onError: () => {
    ...
    const handleDelete = () => {
        ...
        router.delete(`/brand/${dataProps.id}`,  {
            onFinish: () => {
    ...
    const handleDeleteConfirmation = (brand) => {
    ...
    return (
        <AdminLayout title={title}>
                ...
                <table className="w-full text-sm text-left rtl:text-right text-black dark:text-gray-400">
                    ...
                    <tbody>
                        {brands.data.map((brand, i) => (
                            <tr
                                key={i}
                                className={`bg-white border-b dark:bg-gray-800 dark:border-gray-700  ${brand.deleted_at? "line-through bg-yellow-50" : ""}`}
                            >
                                <td
                                    scope="row"
                                    className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white "
                                >
                                    {(brands.current_page - 1) * brands.per_page + i + 1}
                                </td>
                                <td className="px-6 py-4">{brand.name}</td>
                                <td className="px-6 py-4">{brand.note}</td>
                                <td className="px-6 py-4">{brand.user.name}</td>
                                <td className="px-6 py-4">
                                    <div className="flex items-center justify-center gap-2">
                                        <FaPencilAlt
                                            size={20}
                                            color={ brand.deleted_at? "#c2bc42" : "green"}
                                            className="cursor-pointer"
                                            onClick={() => handleShowForm(brand)}
                                        />   
                                        {" "} | {" "}
                                        <FaRegTrashAlt
                                            size={20}
                                            color={ brand.deleted_at? "#e18859" : "red"}
                                            className="cursor-pointer"
                                            onClick={() =>
                                                handleDeleteConfirmation(brand)
                                            }
                                        />
                                    </div>
                                </td>
            ...
            <Pagination data={brands}></Pagination>
        </AdminLayout>
    );
};
