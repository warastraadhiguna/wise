...
const Index = ({ title, units, searchingTextProps }) => {
    ...
    const [perPage, setPerPage]  = useState(units.per_page);
    ...
    const actionForm = (e) => {
        ...
        if (dataProps.id) {
            router.put(`unit/${dataProps.id}`, dataProps, {
                onSuccess: () => {
                    resetForm(true);
                },
                onError: () => {
    ...
    const handleDelete = () => {
        ...
        router.delete(`/unit/${dataProps.id}`,  {
            onFinish: () => {
    ...
    return (
        <AdminLayout title={title}>
                ...
                <table className="w-full text-sm text-left rtl:text-right text-black dark:text-gray-400">
                    ...
                    <tbody>
                        {units.data.map((unit, i) => (
                            <tr
                                key={i}
                                className={`bg-white border-b dark:bg-gray-800 dark:border-gray-700  ${unit.deleted_at? "line-through bg-yellow-50" : ""}`}
                            >
                                <td
                                    scope="row"
                                    className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white "
                                >
                                    {(units.current_page - 1) * units.per_page + i + 1}
                                </td>
                                <td className="px-6 py-4">{unit.name}</td>
                                <td className="px-6 py-4">{unit.note}</td>
                                <td className="px-6 py-4">{unit.user.name}</td>
                                <td className="px-6 py-4">
                                    <div className="flex items-center justify-center gap-2">
                                        <FaPencilAlt
                                            size={20}
                                            color={ unit.deleted_at? "#c2bc42" : "green"}
                                            className="cursor-pointer"
                                            onClick={() => handleShowForm(unit)}
                                        />   
                                        {" "} | {" "}
                                        <FaRegTrashAlt
                                            size={20}
                                            color={ unit.deleted_at? "#e18859" : "red"}
                                            className="cursor-pointer"
                                            onClick={() =>
            ...
            <Pagination data={units}></Pagination>
        </AdminLayout>
    );
};
